...
if balances_response.status_code != 200:
    try:
        print(f"Error fetching balances: {balances_response.json()}")
...
print("Balances response:", balances_data)

for key, value in balances_data.items():
    if key != "SOL":
        order_params = {
            "inputMint": key,  # Mint address
